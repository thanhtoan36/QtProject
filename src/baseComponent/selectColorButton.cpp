...
void SelectColorButton::UpdateButtonStyles()
{
    ...
    QString style_check = QString("QPushButton:checked { background-color:rgb(%1,%2,%3); color:rgb(%4,%5,%6); border-style: solid; border-width: 2px;border-color: rgb(0,176,80);}")
            .arg(BackgroundColor().red()).arg(BackgroundColor().green()).arg(BackgroundColor().blue())
            .arg(TextColor().red()).arg(TextColor().green()).arg(TextColor().blue());
